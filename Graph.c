...
bool hasInfiniteWeights(const Graph *g) {
  if (!g || !g->edges) return false;
  for (unsigned v = 0; v < g->size; v++)
    for (Edge *e = g->edges[v]; e; e = e->next)
...
double calculateAlgebraicConnectivity(const Graph *g) {
  ...
  for (unsigned v = 0; v < n; v++)
    for (Edge *e = g->edges[v]; e; e = e->next)
      if (e->destination < n) {
        L[v][v] += e->weight;
        L[v][e->destination] -= e->weight;
      }
  double epsilon = 1e-9;
  unsigned maxIterations = 100 * n * n;
  for (unsigned i = 0; i < maxIterations; i++) {
    unsigned u = 0, v = 1;
    double maxVal = 0;
    for (unsigned p = 0; p < n; p++)
      for (unsigned q = p + 1; q < n; q++)
        if (fabs(L[p][q]) > maxVal) {
          maxVal = fabs(L[p][q]);
          u = p;
          v = q;
        }
    ...
    double theta = (L[v][v] - L[u][u]) / L[u][v] / 2;
    double t = (theta >= 0) ? 1 / (theta + sqrt(1 + theta * theta)) : -1 / (-theta + sqrt(1 + theta * theta));
    double c = 1 / sqrt(1 + t * t);
    double s = t * c;
    double luu = L[u][u];
    double lvv = L[v][v];
    L[u][u] = c * c * luu - 2 * s * c * L[u][v] + s * s * lvv;
    L[v][v] = s * s * luu + 2 * s * c * L[u][v] + c * c * lvv;
    L[u][v] = 0;
    L[v][u] = 0;
    for (unsigned w = 0; w < n; w++)
      if (w != u && w != v) {
  ...
  double secondSmallest = INFINITY;
  for (unsigned u = 0; u < n; u++)
    if (L[u][u] < smallest) {
      secondSmallest = smallest;
      smallest = L[u][u];
    } else if (L[u][u] < secondSmallest) {
      secondSmallest = L[u][u];
    }
...
double calculateGraphEfficiency(const Graph *g) {
  ...
  for (unsigned u = 0; u < g->size; u++)
    for (Edge *e = g->edges[u]; e; e = e->next)
      if (e->destination < g->size && 0 <= e->weight && e->weight < distance[u][e->destination])
        distance[u][e->destination] = e->weight;
  for (unsigned w = 0; w < g->size; w++)
    for (unsigned u = 0; u < g->size; u++)
      for (unsigned v = 0; v < g->size; v++)
